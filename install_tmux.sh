...
if test -f tmux/.tmux.conf; then
    ...
    curl -s -o $dir1/.tmux.conf https://raw.githubusercontent.com/water-carrier3/dotfiles/main/tmux/.tmux.conf
    file=$dir1/.tmux.conf
fi
...
if [ -x $(command -v nvim) ] && [ -x $(command -v kitty) ]; then
    ...
    if [ "$tmuxx"  == "y" ] || [ -z "$tmuxx" ]; then
        sed -i 's|#set -g @plugin '\''water-carrier3/vim-tmux-kitty-navigator'\''|set -g @plugin '\''water-carrier3/vim-tmux-kitty-navigator'\''|g' ~/.tmux.conf
        if ! grep -q "set -g @plugin 'water-carrier3/vim-tmux-kitty-navigator'" ~/.tmux.conf; then
             echo "set -g @plugin 'water-carrier3/vim-tmux-kitty-navigator'" >> ~/.tmux.conf
        fi
    fi
fi
unset tmuxx
...
if [ -z "$tmuxx" ] || [ "$tmuxx"  == "y" ]; then 
    if test -f tmux/.bash_aliases.d/tmux.sh; then
        ...
        curl -o ~/.bash_aliases.d/tmux.sh https://raw.githubusercontent.com/water-carrier3/dotfiles/main/tmux/.bash_aliases.d/tmux.sh
    fi
