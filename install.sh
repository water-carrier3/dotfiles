...
if ! test -f $HOME/.environment.env; then
    if ! test -f install_envvars.sh; then
        tmp=$(mktemp) && wget -O $tmp https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_envvars.sh
        ./$tmp 'n' 
    else
    ...
    printf "${GREEN}If you want to install applications using appimages, there is a helper called 'appimagelauncher'\n"
     ...
     if test $appimage_install == 'y'; then
         if ! test -f ./install_appimagelauncher.sh; then
            eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_appimagelauncher.sh)" 
         else
...
# Flatpak
   ...
   if [ "y" == "$insflpk" ]; then
       if ! test -f install_flatpak.sh; then
           eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_flatpak.sh)" 
       else
           ./install_flatpak.sh 
       fi 
   fi
...
if ! type snap &> /dev/null; then
    ...
    if [ "y" == "$inssnap" ]; then
        if ! test -f install_snapd.sh; then
            eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_snapd.sh)" 
        else
            ./install_snapd.sh 
        fi 
    fi
...
if test $distro_base == 'Debian'; then
    ...
    if type add-apt-repository &> /dev/null; then
        if ! test -f install_list-ppa.sh; then
                eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_list-ppa.sh)" 
        else
            ./install_list-ppa.sh
        fi
...
if ! sudo test -f /etc/polkit/49-nopasswd_global.pkla && ! sudo test -f /etc/polkit-1/rules.d/90-nopasswd_global.rules; then
    ...
    if [ "y" == "$plkit" ]; then
        if ! test -f install_polkit_wheel.sh; then
            eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_polkit_wheel.sh)" 
        else
...
if ! test -f ~/.bash_completion.d/complete_alias || ! test -f /root/.bash_completion.d/complete_alias; then
    ...
    if [ -z "$compl" ] || [ "y" == "$compl" ]; then
        if ! test -f install_bashalias_completions.sh; then
            eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_bashalias_completions.sh)" 
        else
...
if ! type activate-global-python-argcomplete &> /dev/null; then
    ...
    if [ -z $pycomp ] || [ "y" == $pycomp ]; then
        if ! test -f install_python_completions.sh; then
            eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_python_completions.sh)" 
        else
...
if [ -z $scripts ] || [ "y" == $scripts ]; then
    ...
    if ! test -f install_aliases.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/install_aliases.sh)" 
    else
        ./install_aliases.sh
    fi
fi
...
if ! test -f $binds; then
    tmp=$(mktemp) && curl -o $tmp https://raw.githubusercontent.com/water-carrier3/dotfiles/main/keybinds/.inputrc
    tmp1=$(mktemp) && curl -o $tmp https://raw.githubusercontent.com/water-carrier3/dotfiles/main/keybinds/.keybinds.d/keybinds.bash 
    tmp2=$(mktemp) && curl -o $tmp https://raw.githubusercontent.com/water-carrier3/dotfiles/main/keybinds/.keybinds 
    binds=$tmp
    binds1=$tmp1
    binds2=$tmp2
fi
...
if ! test -f xterm/.Xresources; then
    tmp=$(mktemp) && curl -o $tmp https://raw.githubusercontent.com/water-carrier3/dotfiles/main/xterm/.Xresources
    xterm=$tmp
fi
...
if ! test -f ~/.bash_preexec || ! test -f /root/.bash_preexec; then
    ...
    if [ -z "$bash_preexec" ] || [ "y" == "$bash_preexec" ]; then
        if ! test -f install_bash_preexec.sh; then
            eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_bash_preexec.sh)" 
        else
...
if [ $pipew == "y" ]; then
    if ! test -f install_pipewire.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_pipewire.sh)" 
    else
        ./install_pipewire.sh 
    fi
fi
...
if [ -z $moar ] || [ "Y" == $moar ] || [ $moar == "y" ]; then
    if ! test -f install_moar.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_moar.sh)" 
    else
        ./install_moar.sh 
    fi
fi
...
if [ "y" == "$nno" ]; then
    if ! test -f install_nano.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_nano.sh)" 
    else
        ./install_nano.sh
    fi
fi
...
if [ "y" == "$nvm" ]; then
    if ! test -f install_nvim.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_nvim.sh)" 
    else
        ./install_nvim.sh
    fi
fi
...
if [ "y" == "$kittn" ]; then
    if ! test -f install_kitty.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_kitty.sh)" 
    else
        ./install_kitty.sh
    fi
fi
...
if [ "y" == "$findr" ]; then
    if ! test -f install_fzf.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_fzf.sh)" 
    else
        ./install_fzf.sh
    fi
fi
...
if [ "y" == "$git_ins" ]; then
    if ! test -f install_git.sh; then
        ins_git=$(mktemp)
        curl -o $ins_git https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_git.sh   
        chmod u+x "$ins_git"
        eval "$ins_git" 'global'
    else
...
if [ "y" == "$git_ins" ]; then
    if ! test -f install_lazygit.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_lazygit.sh)" 
    else
        ./install_lazygit.sh
    fi
fi
...
if [ "y" == "$rngr" ]; then
    if ! test -f install_ranger.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_ranger.sh)" 
    else
        ./install_ranger.sh
    fi
fi
...
if [ "y" == "$tmx" ]; then
    if ! test -f install_tmux.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_tmux.sh)" 
    else
        ./install_tmux.sh
    fi
fi
...
if [ "$tojump" == "y" ]; then
    if ! test -f install_neofetch_onefetch.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_neofetch_onefetch.sh)" 
    else
...
if [ "$tojump" == "y" ]; then
    if ! test -f install_bashtop.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_bashtop.sh)" 
    else
        ./install_bashtop.sh
    fi
fi
...
if type autojump &> /dev/null; then
    ...
    if [ "$tojump" == "y" ]; then
        if ! test -f install_autojump.sh; then
            eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_autojump.sh)" 
        else
            ./install_autojump.sh
        fi
    fi
...
if [ $strshp == "y" ]; then
    if ! test -f install_starship.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_starship.sh)" 
    else
        ./install_starship.sh 
    fi
fi
...
if ! type nmap &> /dev/null; then
    ...
    if [ "$tojump" == "y" ]; then
        if ! test -f install_nmap.sh; then
            eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_nmap.sh)" 
        else
            ./install_nmap.sh
        fi
    fi
...
if [ $ins_ufw == "y" ]; then
    if ! test -f install_ufw.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_ufw.sh)" 
    else
        ./install_ufw.sh 
    fi
fi
...
#        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_netstat.sh)" 
#    else
#        ./install_netstat.sh
#    fi
#fi
...
if [ "y" == "$git_ins" ]; then
    if ! test -f install_lazydocker.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_lazydocker.sh)" 
    else
...
if [ -z $exif_t ] || [ "Y" == $exif_t ] || [ $exif_t == "y" ]; then
    if ! test -f install_exiftool.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_exiftool.sh)" 
    else
        ./install_exiftool.sh 
    fi
fi
...
if [ "y" == "$kittn" ]; then
    if ! test -f install_testdisk.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_testdisk.sh)" 
    else
        ./install_testdisk.sh
    fi
fi
...
if [ "$ffmpg" == "y" ]; then
    if ! test -f install_ffmpeg.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_ffmpeg.sh)" 
    else
        ./install_ffmpeg.sh
    fi
fi
...
if type yt-dlp &> /dev/null; then
    ...
    if [ "$tojump" == "y" ]; then
        if ! test -f install_yt-dlp.sh; then
            eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_yt-dlp.sh)" 
        else
            ./install_yt-dlp.sh
        fi
    fi
...
#reade -Q "$color" -i "$pre" -p "Check existence/create .environment.env and link it to .bashrc in $HOME/ and /root/? $prmpt" "$othr" envvars
    ...
    if ! test -f install_envvars.sh; then
        tmp=$(mktemp) 
        wget -O $tmp https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_envvars.sh
        ./$tmp  
    else
        ./install_envvars.sh  
    fi 
