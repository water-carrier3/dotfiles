    ...
    if test "$langs" == 'y'; then
            ...
            if [ -z $pyscripts ] || [ "y" == $pyscripts ]; then
                if ! type pyenv &> /dev/null; then 
                    if ! test -f install_pyenv.sh; then
                         eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_pyenv.sh)" 
                    else
                        ./install_pyenv.sh
                    fi
                fi 
        ...
        if ! type gem &> /dev/null || ! gem list | grep neovim &> /dev/null; then
            ...
            if [ -z $rubyscripts ] || [ "y" == $rubyscripts ]; then
                ...
                if ! test -f install_ruby.sh; then
                     eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_ruby.sh)" 
                else
                    ./install_ruby.sh
                fi
                gem install neovim
        ...
        if ! type cpanm &> /dev/null; then
            if ! test -f install_cpanm.sh; then
                 eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_cpanm.sh)" 
            else
                ./install_cpanm.sh
            fi 
        fi
...
if ! test -d vim/; then
    tmpdir=$(mktemp -d -t nvim-XXXXXXXXXX)
    tmpfile=$(mktemp)
    curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/download_git_directory.sh | tee "$tmpfile" &> /dev/null
    chmod u+x "$tmpfile"
    eval $tmpfile https://github.com/water-carrier3/dotfiles/tree/main/vim/.config/nvim $tmpdir
...
if ! test -d vim/.bash_aliases.d/ || ! test -d vim/.bash_completions.d/; then
    tmp=$(mktemp) && curl -o $tmp https://raw.githubusercontent.com/water-carrier3/dotfiles/main/vim/.bash_aliases.d/vim_nvim.sh
    tmp1=$(mktemp) && curl -o $tmp1 https://raw.githubusercontent.com/water-carrier3/dotfiles/main/vim/.bash_completions.d/vim_nvim
    file=$tmp
    file1=$tmp1
fi
...
if ! type nvimpager &> /dev/null; then
    ...
    if [ -z "$vimrc" ] || [ "$vimrc" == "y" ]; then
        if ! test -f install_nvimpager.sh; then
             eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_nvimpager.sh)" 
        else
