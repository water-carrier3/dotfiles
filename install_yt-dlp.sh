...
if ! type ffmpeg &> /dev/null; then
    ...
    if test $ffmpeg == 'y' ;then
        if ! test -f install_ffmpeg.sh; then
            eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/install_ffmpeg.sh)" 
        else
            ./install_ffmpeg.sh
        fi
    fi
...
if ! test -d aliases/.bash_aliases.d/; then 
    wget -P $TMPDIR/ https://raw.githubusercontent.com/water-carrier3/dotfiles/main/aliases/.bash_aliases.d/youtube.sh 
