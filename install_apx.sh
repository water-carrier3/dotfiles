...
if ! test -f install_docker.sh; then
     eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_docker.sh)" 
else
    ./install_docker.sh
fi
...
if [ $distro == "Manjaro" ]; then
    ...
    if ! test -f install_distrobox.sh; then
        eval "$(curl -fsSL https://raw.githubusercontent.com/water-carrier3/dotfiles/main/install_distrobox.sh)" 
     else
